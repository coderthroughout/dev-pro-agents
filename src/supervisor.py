...
class Supervisor:
    ...
    def _analyze_execution_results(self, result: dict[str, Any]) -> bool:
        ...
        # Simple heuristic: look for success indicators in messages
        final_message = str(messages[-1]).lower()
        success_indicators = ["completed", "finished", "success", "done"]
        error_indicators = ["error", "failed", "exception", "blocked"]

        has_success = any(
            indicator in final_message for indicator in success_indicators
        )
        has_error = any(indicator in final_message for indicator in error_indicators)
