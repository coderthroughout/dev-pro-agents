...
class FirecrawlSettings(APIClientSettings):
    """Firecrawl API configuration for web scraping and crawling."""

    model_config = SettingsConfigDict(env_prefix="FIRECRAWL_")

    api_key: str = Field(..., description="Firecrawl API key")
    base_url: HttpUrl = Field(
        "https://api.firecrawl.dev/v1", description="Firecrawl API base URL"
    )
    # Extended timeout for crawling operations
    timeout_seconds: int = Field(
        120, ge=1, le=600, description="Extended timeout for crawling operations"
    )
    ...
    crawl_limit: int = Field(
        100, ge=1, le=1000, description="Default limit for crawl operations"
    )
    max_wait_time: int = Field(
        600,
        ge=30,
        le=3600,
        description="Maximum wait time for crawl completion (seconds)",
    )
    poll_interval: int = Field(
        10,
        ge=1,
        le=60,
        description="Polling interval for crawl status checks (seconds)",
    )
...
def get_firecrawl_config() -> dict[str, Any]:
    """Get Firecrawl client configuration."""
    return settings.get_api_client_config("firecrawl")
...
def validate_configuration() -> bool:
    ...
    try:
        ...
        required_api_keys = [
            ...
            ("Firecrawl", test_settings.firecrawl.api_key),
        ]
